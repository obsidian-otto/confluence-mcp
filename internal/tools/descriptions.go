...
` + templates.Backtick + `Wire flow (up to four calls, mirrors a native owning diagram):` + templates.Backtick + ` Step 1 creates the page when ` + templates.Backtick + `spaceId` + templates.Backtick + ` + ` + templates.Backtick + `title` + templates.Backtick + ` are supplied. Step 2 uploads the source through the v1 attachment endpoint with an extension-derived Content-Type (` + templates.Backtick + `.drawio` + templates.Backtick + ` → ` + templates.Backtick + `application/vnd.jgraph.mxfile` + templates.Backtick + `). Step 3 creates the ` + templates.Backtick + `ac:com.mxgraph.confluence.plugins.diagramly:drawio-diagram` + templates.Backtick + ` custom-content entity with URL-encoded metadata JSON. Step 4 PUTs an owning ` + templates.Backtick + `drawio` + templates.Backtick + ` macro on the same page. This distinction is load-bearing: ` + templates.Backtick + `drawio` + templates.Backtick + ` opens the editor; ` + templates.Backtick + `inc-drawio` + templates.Backtick + ` is for embedding a diagram owned by another page and opens the viewer.

` + templates.Backtick + `Macro envelope emitted:` + templates.Backtick + ` Native owning-page ` + templates.Backtick + `ac:name="drawio"` + templates.Backtick + ` contract with fresh ` + templates.Backtick + `ac:local-id` + templates.Backtick + ` / ` + templates.Backtick + `ac:macro-id` + templates.Backtick + ` UUIDs and the app metadata ` + templates.Backtick + `mVer` + templates.Backtick + `, ` + templates.Backtick + `inComment` + templates.Backtick + `, ` + templates.Backtick + `pageId` + templates.Backtick + `, ` + templates.Backtick + `custContentId` + templates.Backtick + `, ` + templates.Backtick + `diagramDisplayName` + templates.Backtick + `, ` + templates.Backtick + `diagramName` + templates.Backtick + `, ` + templates.Backtick + `contentVer` + templates.Backtick + `, ` + templates.Backtick + `revision` + templates.Backtick + `, and ` + templates.Backtick + `baseUrl` + templates.Backtick + `. Preview/aspect attachments remain draw.io runtime output after the first editor save.
