...
` + templates.Backtick + `Modes:` + templates.Backtick + ` Static-PNG (default, works without the drawio marketplace app) and Editable (requires the marketplace app). For ` + templates.Backtick + `.drawio` + templates.Backtick + ` inputs the tool wraps the XML into a ` + templates.Backtick + `.drawio.png` + templates.Backtick + ` using the same algorithm as hunyadi/md2conf (base64 → raw DEFLATE → URL-encode → embed in PNG ` + templates.Backtick + `tEXt` + templates.Backtick + ` chunk with keyword ` + templates.Backtick + `mxfile` + templates.Backtick + `). For ` + templates.Backtick + `.drawio.png` + templates.Backtick + ` inputs the file is uploaded as-is. For ` + templates.Backtick + `.drawio.svg` + templates.Backtick + ` inputs the file is uploaded as-is (the ` + templates.Backtick + `content` + templates.Backtick + ` attribute on the root ` + templates.Backtick + `<svg>` + templates.Backtick + ` carries the embedded drawio XML). If the marketplace app is installed, drawio extracts the XML and renders the editable diagram; if not, Confluence's stock renderer just shows the PNG or SVG.

` + templates.Backtick + `Wire flow (2 steps, not 1):` + templates.Backtick + ` Step 1 uploads the file via the v1 endpoint (POST ` + templates.Backtick + `/wiki/rest/api/content/{pageId}/child/attachment` + templates.Backtick + ` with multipart/form-data + ` + templates.Backtick + `X-Atlassian-Token: no-check` + templates.Backtick + `). Step 2 either creates a new page (POST v2 ` + templates.Backtick + `/wiki/api/v2/pages` + templates.Backtick + ` when ` + templates.Backtick + `spaceId` + templates.Backtick + ` is set) or updates the existing page body (PUT v2 ` + templates.Backtick + `/wiki/api/v2/pages/{id}` + templates.Backtick + ` when ` + templates.Backtick + `pageId` + templates.Backtick + ` is set) to add the ` + templates.Backtick + `<ac:structured-macro ac:name="drawio">` + templates.Backtick + ` block referencing the attachment by ` + templates.Backtick + `diagramName` + templates.Backtick + `.

` + templates.Backtick + `Macro envelope emitted:` + templates.Backtick + ` Minimal form: ` + templates.Backtick + `diagramName` + templates.Backtick + ` + ` + templates.Backtick + `width` + templates.Backtick + ` + ` + templates.Backtick + `height` + templates.Backtick + ` + ` + templates.Backtick + `baseUrl` + templates.Backtick + `. If the marketplace app is installed, Confluence's editor auto-fills the rich parameter set (` + templates.Backtick + `contentId` + templates.Backtick + `, ` + templates.Backtick + `pageId` + templates.Backtick + `, ` + templates.Backtick + `revision` + templates.Backtick + `) on next edit.
...
` + templates.Backtick + `Returns:` + templates.Backtick + ` Envelope ` + templates.Backtick + `{attachmentId, attachmentTitle, attachmentVersion, diagramName, page: {id, title, version}}` + templates.Backtick + `. Use ` + templates.Backtick + `jq: "{attachmentId: attachmentId, pageId: page.id}"` + templates.Backtick + ` to extract the two ids for follow-up operations.`
