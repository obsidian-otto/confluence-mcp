...
` + templates.Backtick + `Filters:` + templates.Backtick + ` ` + templates.Backtick + `mediaType` + templates.Backtick + ` is an EXACT match against the attachment's mediaType (e.g. ` + templates.Backtick + `"image/png"` + templates.Backtick + ` — NOT a substring like ` + templates.Backtick + `"image"` + templates.Backtick + `). ` + templates.Backtick + `filename` + templates.Backtick + ` is exact match (case-sensitive).

` + templates.Backtick + `Output format:` + templates.Backtick + ` TOON (default) or JSON (` + templates.Backtick + `outputFormat: "json"` + templates.Backtick + `).

` + templates.Backtick + `Returns:` + templates.Backtick + ` v2 ` + templates.Backtick + `MultiEntityResult<Attachment>` + templates.Backtick + ` envelope (` + templates.Backtick + `results: [{id, title, mediaType, fileSize, ...}], _links.next` + templates.Backtick + `). Use ` + templates.Backtick + `jq: "results[*].{id: id, title: title, mediaType: mediaType, fileSize: fileSize}"` + templates.Backtick + ` to extract a compact list. NOTE: the upload endpoint returns a v1 envelope where ` + templates.Backtick + `mediaType` + templates.Backtick + ` and ` + templates.Backtick + `fileSize` + templates.Backtick + ` live under ` + templates.Backtick + `extensions` + templates.Backtick + ` — different shape from this v2 endpoint.`
...
// CONF_UPLOAD_DRAWIO_DESCRIPTION documents the conf_upload_drawio
// tool. Local addition — the upstream has no drawio tool — so
// the upstream-drift guardrail does not apply.
const CONF_UPLOAD_DRAWIO_DESCRIPTION = `Upload a drawio diagram to Confluence and embed it on a page with conf_upload_drawio. Returns TOON format by default.

` + templates.Backtick + `Use this when:` + templates.Backtick + ` You have a drawio diagram on disk (a ` + templates.Backtick + `.drawio` + templates.Backtick + ` XML file, a ` + templates.Backtick + `.drawio.png` + templates.Backtick + ` PNG with embedded XML, or a ` + templates.Backtick + `.drawio.svg` + templates.Backtick + ` SVG with embedded XML) and want to upload it to a Confluence page with the diagram rendered inline. This is the single tool that does the full upload-AND-embed flow; the underlying multipart POST and page body PUT are not exposed to the caller.

` + templates.Backtick + `NOT for general image uploads:` + templates.Backtick + ` For PNG, JPEG, SVG (without embedded drawio XML), PDF, DOCX, or any other binary attachment that is NOT a drawio diagram, use ` + templates.Backtick + `conf_upload_attachment` + templates.Backtick + ` — it accepts any binary format (PNG, PDF, JPEG, SVG, ZIP, etc.) and uploads it without the drawio-specific wrapping or macro insertion. conf_upload_drawio is ONLY for files whose content is a drawio diagram.

` + templates.Backtick + `Modes:` + templates.Backtick + ` Editable (raw .drawio source attachment, the format the draw.io marketplace app expects on Confluence Cloud) and Static-PNG (a pre-prepared .drawio.png with the drawio XML embedded in a ` + templates.Backtick + `tEXt` + templates.Backtick + ` chunk — keyword ` + templates.Backtick + `mxfile` + templates.Backtick + `). For ` + templates.Backtick + `.drawio` + templates.Backtick + ` inputs the tool uploads the source XML verbatim. For ` + templates.Backtick + `.drawio.png` + templates.Backtick + ` inputs the file is uploaded as-is. For ` + templates.Backtick + `.drawio.svg` + templates.Backtick + ` inputs the file is uploaded as-is (the ` + templates.Backtick + `content` + templates.Backtick + ` attribute on the root ` + templates.Backtick + `<svg>` + templates.Backtick + ` carries the embedded drawio XML).

` + templates.Backtick + `Wire flow (1-3 calls, mirrors the web-UI drag-drop):` + templates.Backtick + ` Step 1 creates the page (only when ` + templates.Backtick + `spaceId` + templates.Backtick + ` + ` + templates.Backtick + `title` + templates.Backtick + ` are supplied; an empty body so the draw.io app can insert its own macros on first edit). Step 2 uploads the source file via the v1 endpoint (POST ` + templates.Backtick + `/wiki/rest/api/content/{pageId}/child/attachment` + templates.Backtick + ` with multipart/form-data + ` + templates.Backtick + `X-Atlassian-Token: no-check` + templates.Backtick + `) — the file part's Content-Type is derived from the extension (.drawio → ` + templates.Backtick + `application/vnd.jgraph.mxfile` + templates.Backtick + `). Step 3 creates the ` + templates.Backtick + `ac:com.mxgraph.confluence.plugins.diagramly:drawio-diagram` + templates.Backtick + ` custom-content entity on the page (POST ` + templates.Backtick + `/wiki/api/v2/custom-content` + templates.Backtick + `) — the draw.io app finds the diagram via this entity. CRITICAL: the body.value must be URL-encoded JSON (` + templates.Backtick + `%7B%22pageId%22...` + templates.Backtick + `), not raw JSON or HTML-escaped JSON — the draw.io app's body parser calls ` + templates.Backtick + `decodeURIComponent(value)` + templates.Backtick + ` first, then ` + templates.Backtick + `JSON.parse` + templates.Backtick + `. Step 4 puts the inc-drawio macro (ac:name="inc-drawio") on the page (PUT ` + templates.Backtick + `/wiki/api/v2/pages/{id}` + templates.Backtick + `) — this is the trigger that fires the draw.io app's read-view renderer and the "Edit this diagram" affordance. The macro envelope mirrors what the draw.io app writes on first save of a page in the draw.io Diagrams app.

` + templates.Backtick + `Macro envelope emitted:` + templates.Backtick + ` Full native contract matching what the draw.io app's own upload flow produces on first save of a draw.io diagram. The draw.io app's read-view renderer hooks into this macro (ac:name="inc-drawio") and fires the diagram inline. The full parameter set (pageId, custContentId, diagramDisplayName, diagramName, imgPageId, width, height, simple, zoom, lbox, hiResPreview, pCenter, links, tbstyle, includedDiagram, revision, baseUrl) is what the draw.io app expects for the "Edit this diagram" affordance to recognise the diagram as fully-formed. The remaining parameters on the working DrawIO Test page (aspect, aspectHash, contentVer, contentId) are runtime-computed by the draw.io app on first save — they're not part of the source macro and don't need to be set.

` + templates.Backtick + `Arg rules:` + templates.Backtick + ` Exactly one of ` + templates.Backtick + `pageId` + templates.Backtick + ` (existing page) or ` + templates.Backtick + `spaceId` + templates.Backtick + ` + ` + templates.Backtick + `title` + templates.Backtick + ` (new page) must be set. Exactly one of ` + templates.Backtick + `drawioFile` + templates.Backtick + `, ` + templates.Backtick + `drawioPngFile` + templates.Backtick + `, or ` + templates.Backtick + `drawioSvgFile` + templates.Backtick + ` must be set; precedence order is ` + templates.Backtick + `drawioFile` + templates.Backtick + ` > ` + templates.Backtick + `drawioPngFile` + templates.Backtick + ` > ` + templates.Backtick + `drawioSvgFile` + templates.Backtick + ` if multiple are provided.

` + templates.Backtick + `When NOT to use this:` + templates.Backtick + ` For Confluence attachments that are NOT drawio diagrams (PNG, PDF, DOCX, etc.) use ` + templates.Backtick + `conf_upload_attachment` + templates.Backtick + ` and then edit the page body separately. For updating an EXISTING drawio attachment (rather than uploading a new one), use ` + templates.Backtick + `conf_upload_attachment` + templates.Backtick + ` with the same filename — Confluence treats that as a new version.

` + templates.Backtick + `Output format:` + templates.Backtick + ` TOON (default) or JSON (` + templates.Backtick + `outputFormat: "json"` + templates.Backtick + `).

` + templates.Backtick + `Returns:` + templates.Backtick + ` Envelope ` + templates.Backtick + `{attachmentId, attachmentTitle, attachmentVersion, customContentId, customContentVersion, diagramName, page: {id, title, version}}` + templates.Backtick + `. Use ` + templates.Backtick + `jq: "{attachmentId: attachmentId, customContentId: customContentId, pageId: page.id}"` + templates.Backtick + ` to extract the two ids for follow-up operations.`
