...
// newGetCmd returns the `get` subcommand. It maps
...
EXAMPLES:
  ...
  mcp-confluence get --path=/wiki/api/v2/spaces?limit=2

  # Get a single page's storage body, extract just the markdown via jq:
  mcp-confluence get --path=/wiki/api/v2/pages/163935/body --jq=results.value
...
AUTOMATION:
  ...
  #
  #   list-spaces:
  #       mcp-confluence get --path=/wiki/api/v2/spaces?limit=$$LIMIT
  #
  # Or from a shell script that pipes to jq:
  #
  #   mcp-confluence get --path=/wiki/api/v2/spaces \
  #       --jq='results[*].{id: id, key: key, name: name}'`,
	...
	if err := registerFlagsFromArgsStruct(cmd, args); err != nil {
		...
		panic("get: registerFlagsFromArgsStruct: " + err.Error())
	}
	return cmd
}

// newPostCmd returns the `post` subcommand. POST takes
...
EXAMPLES:
  ...
  mcp-confluence post --path=/wiki/api/v2/pages \
...
AUTOMATION:
  ...
  #               '{spaceId: "780763211", status: "current", title: $$title, body: {representation: "storage", value: $$body}}')"`,
	...
	if err := registerFlagsFromArgsStruct(cmd, args); err != nil {
		panic("post: registerFlagsFromArgsStruct: " + err.Error())
	}
	return cmd
}

// newPutCmd returns the `put` subcommand. PUT is a
...
incremented by 1). For partial updates use patch.

USAGE:
  mcp-confluence put [flags]
...
EXAMPLES:
  ...
  mcp-confluence put --path=/wiki/api/v2/pages/163935 \
...
AUTOMATION:
  ...
  #
  #   rewrite-doc:
  #       mcp-confluence put --path=/wiki/api/v2/pages/$$PAGE_ID \
  #           --body="$$(cat /tmp/page-payload.json)"`,
	...
	if err := registerFlagsFromArgsStruct(cmd, args); err != nil {
		panic("put: registerFlagsFromArgsStruct: " + err.Error())
	}
	return cmd
}

// newPatchCmd returns the `patch` subcommand. PATCH
...
AUTOMATION:
  ...
  #
  #   bump-title:
  #       mcp-confluence patch --path=/wiki/api/v2/pages/$$PAGE_ID \
  #           --body="[{\"op\":\"replace\",\"path\":\"/title\",\"value\":\"$$NEW_TITLE\"}]"`,
	...
	if err := registerFlagsFromArgsStruct(cmd, args); err != nil {
		panic("patch: registerFlagsFromArgsStruct: " + err.Error())
	}
	return cmd
}
...
// OutputFormat — same shape as get.
...
EXAMPLES:
  # Delete a page by id:
  mcp-confluence delete --path=/wiki/api/v2/pages/163935

  # Delete a label from a page (path includes the label id):
  mcp-confluence delete --path=/wiki/api/v2/pages/163935/labels/789
...
AUTOMATION:
  ...
  #           mcp-confluence delete --path=/wiki/api/v2/pages/$$id; \
  #       done`,
		RunE: func(cmd *cobra.Command, _ []string) error {
	...
	if err := registerFlagsFromArgsStruct(cmd, args); err != nil {
		panic("delete: registerFlagsFromArgsStruct: " + err.Error())
	}
	return cmd
}
