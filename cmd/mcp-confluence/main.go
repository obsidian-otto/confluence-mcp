...
JSON-RPC stream when running in stdio mode.`,
	...
	root.AddCommand(newListSpacesCmd())
	root.AddCommand(newListPagesCmd())
	root.AddCommand(newGetPageBodyCmd())
	root.AddCommand(newGetPageTreeCmd())
	root.AddCommand(newSearchCmd())
	root.AddCommand(newHelpCmd())
	// 3 markdown (Phase 21) — round-trip via internal/markdown
	root.AddCommand(newPostMarkdownCmd())
	root.AddCommand(newPutMarkdownCmd())
	root.AddCommand(newGetPageMarkdownCmd())
	// 3 attachments (Phase 21) — v1 upload + v2 list/delete
	root.AddCommand(newUploadAttachmentCmd())
	root.AddCommand(newListAttachmentsCmd())
	root.AddCommand(newDeleteAttachmentCmd())
	// 1 drawio (Phase 21) — upload + embed in one call
	root.AddCommand(newUploadDrawioCmd())
